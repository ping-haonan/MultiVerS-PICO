...
class Corpus:
    ...
    def get_document(self, doc_id):
        ...
        if len(res) == 1:
            return res[0]
        # Try string match
        res_str = [x for x in self.documents if str(x.id) == str(doc_id)]
        if len(res_str) >= 1:
            return res_str[0]
        return None
...
class GoldDataset:
    ...
    def _read_claims(self, data_file):
        ...
        for this_example in examples:
            ...
            # Compatibility: Handle both 'cited_doc_ids' and 'doc_ids'
            doc_ids_key = "cited_doc_ids" if "cited_doc_ids" in entry else "doc_ids"
            if doc_ids_key not in entry:
                # If neither exists, assume empty (e.g., test set)
                entry[doc_ids_key] = []
                
            # Handle potential missing documents
            entry["cited_docs"] = []
            for doc in entry[doc_ids_key]:
                found_doc = self.corpus.get_document(doc)
                if found_doc is not None:
                    entry["cited_docs"].append(found_doc)
            
            # Normalize key to cited_doc_ids if we used doc_ids
            if doc_ids_key == "doc_ids":
                entry["cited_doc_ids"] = entry.pop("doc_ids")
                
            # assert len(entry["cited_docs"]) == len(entry["cited_doc_ids"])
            del entry["cited_doc_ids"]
            
            # Fix missing evidence field (e.g. in SciFact-Open)
            if "evidence" not in entry:
                entry["evidence"] = {}
            
            # Remove extra fields not in Claim dataclass
            if "claim_original" in entry:
                del entry["claim_original"]
