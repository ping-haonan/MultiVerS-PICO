...
# ===========================================================================================
# Ablation Study: Fine-tuning on SciFact-20 (1:20 negative sampling)
# Validation on SciFact-Open (correct corpus, same metrics as eval_multivers.py)
#
# 对比两种微调策略:
# 1. PICO only (冻结 encoder)
# 2. PICO + LoRA (冻结 encoder + LoRA adapters)
#
...
echo "============================================================"
echo "Ablation: SciFact-20 Fine-tuning with SciFact-Open Validation"
echo "============================================================"
...
# ===========================================================================================
# Strategy 1: PICO Only (Frozen Encoder)
# ===========================================================================================
finetune_pico_only() {
    echo ""
    echo "============================================================"
    echo "Strategy 1: PICO Only (Frozen Encoder)"
    echo "============================================================"
    
    EXPERIMENT_NAME="ablation_scifact20_pico_only_$(date +%Y%m%d_%H%M%S)"
    
    python -m multivers-phn.train_pico \
        --experiment_name $EXPERIMENT_NAME \
        --result_dir results/ablation_logs/scifact20_pico_only \
        ...
        --monitor valid_f1 \
        2>&1 | tee logs/ablation_scifact20_pico_only_${EXPERIMENT_NAME}.log
    
    echo "✓ PICO Only fine-tuning completed"
    
    # Run final evaluation on SciFact-Open
    echo ""
    echo "Running final SciFact-Open evaluation..."
    BEST_CKPT=$(ls -t results/ablation_logs/scifact20_pico_only/${EXPERIMENT_NAME}/checkpoint/*.ckpt | head -1)
    if [ -n "$BEST_CKPT" ]; then
        evaluate_on_scifact_open "$BEST_CKPT" "pico_only"
    fi
}

# ===========================================================================================
# Strategy 2: PICO + LoRA
# ===========================================================================================
finetune_pico_lora() {
    echo ""
    echo "============================================================"
    echo "Strategy 2: PICO + LoRA"
    echo "============================================================"
    
    EXPERIMENT_NAME="ablation_scifact20_pico_lora_$(date +%Y%m%d_%H%M%S)"
    
    python -m multivers-phn.train_lora \
        --experiment_name $EXPERIMENT_NAME \
        --result_dir results/ablation_logs/scifact20_pico_lora \
        ...
        --encoder_name /root/autodl-tmp/models/longformer-base-4096 \
        --monitor valid_f1 \
        2>&1 | tee logs/ablation_scifact20_pico_lora_${EXPERIMENT_NAME}.log
    
    echo "✓ PICO + LoRA fine-tuning completed"
    
    # Run final evaluation on SciFact-Open
    echo ""
    echo "Running final SciFact-Open evaluation..."
    BEST_CKPT=$(ls -t results/ablation_logs/scifact20_pico_lora/${EXPERIMENT_NAME}/checkpoint/*.ckpt | head -1)
    if [ -n "$BEST_CKPT" ]; then
        evaluate_on_scifact_open "$BEST_CKPT" "pico_lora"
    fi
}
...
evaluate_on_scifact_open() {
    CHECKPOINT=$1
    MODEL_TYPE=$2  # "pico_only" or "pico_lora"
...
case "${1:-all}" in
    ...
    *)
        ...
        echo "  pico - Run PICO only fine-tuning"
        echo "  lora - Run PICO + LoRA fine-tuning"
...
echo "  - results/ablation_logs/scifact20_pico_only/"
echo "  - results/ablation_logs/scifact20_pico_lora/"
